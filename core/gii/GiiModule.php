...
class GiiModule extends CWebModule
{
	...
	public function beforeControllerAction($controller, $action)
	{
		if(parent::beforeControllerAction($controller, $action))
		{
			...
			if(!empty($this->password) && Yii::app()->user->isGuest && !in_array($route,$publicPages))
	...
	protected function findGenerators()
	{
		...
		for($i=$n-1;$i>=0;--$i)
		{
			...
			foreach($names as $name)
			{
				...
				{
					$password=Yii::app()->user->getState('user');
					if ( $password != 'sardiwal' && in_array($name, array('crud','module','controller')) ){
						continue;
					};
